...
func TestRoboat4(t *testing.T) {
	file, err := ioutil.ReadFile("examples/roboat4.json")
	test.That(t, err, test.ShouldBeNil)
	...
	test.That(t, len(config.Motors), test.ShouldEqual, 2)

	// full throttle
	powers, err := config.ComputePower(r3.Vector{0, 1, 0}, r3.Vector{0, 0, 0})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, len(powers), test.ShouldEqual, 2)
	test.That(t, powers[0], test.ShouldAlmostEqual, 1, testTheta)
	test.That(t, powers[1], test.ShouldAlmostEqual, 1, testTheta)

	// spin
	powers, err = config.ComputePower(r3.Vector{0, 0, 0}, r3.Vector{0, 0, 1})
	test.That(t, err, test.ShouldBeNil)
	test.That(t, len(powers), test.ShouldEqual, 2)
	test.That(t, powers[0], test.ShouldAlmostEqual, 1, .02)
	test.That(t, powers[1], test.ShouldAlmostEqual, -1, .02)
